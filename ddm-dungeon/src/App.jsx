...
// ─── FALLBACK ROOMS (used if backend is unreachable) ─────────────────────────
...
const SECTION_COLORS = {
  ...
  ddm: {
    bg: "rgba(35,15,40,0.5)",
    border: "rgba(180,80,220,0.25)",
    title: "#cc5de8",
  },
  bayesian: {
    bg: "rgba(15,35,25,0.5)",
    border: "rgba(80,200,120,0.25)",
    title: "#51cf66",
  },
  resource_rationality: {
    bg: "rgba(40,35,10,0.5)",
    border: "rgba(220,180,40,0.25)",
    title: "#fcc419",
  },
};

function ExplanationPanel({ sections, onContinue }) {
  if (!sections || sections.length === 0) return null;
  return (
    <div style={{ animation: "fadeSlideUp 0.4s ease both", marginTop: "18px" }}>
      <div
        style={{
          color: "rgba(180,100,20,0.5)",
          fontSize: "9px",
      ...
      {sections.map((s, i) => {
        const colors = SECTION_COLORS[s.type] || SECTION_COLORS.verdict;
        return (
          <div
            key={i}
            style={{
      ...
      <button
        ...
        style={{
          ...
          marginTop: "10px",
          background:
            "linear-gradient(135deg, rgba(160,70,0,0.2), rgba(100,40,0,0.12))",
        ...
        onMouseEnter={(e) => {
          e.currentTarget.style.boxShadow = "0 0 25px rgba(200,100,0,0.2)";
...
function RoomView({ room, roomIdx, totalRooms, onDecide }) {
  const [phase, setPhase] = useState("evidence"); // evidence | choosing | done | explanation
  ...
  const handleChoice = async (idx, timedOut = false) => {
    if (phase === "done" || phase === "explanation") return;
    clearInterval(timerRef.current);
    ...
    decisionRef.current = {
      ...
      choseOptimal,
      rt,
      drift_signal: room.drift_signal,
      chosenEV: room.choices[idx].ev,
    ...
    try {
      const res = await fetch(`${API_BASE}/explain`, {
        ...
        body: JSON.stringify({ roomId: room.id, choiceIdx: idx, rt }),
      });
      if (res.ok) {
      ...
      // If backend unreachable, no explanation (just show feedback)
    } finally {
      setLoadingExpl(false);
    }

    // Short delay then switch to explanation phase
    setTimeout(() => setPhase("explanation"), 900);
  };

  const handleContinue = () => {
    if (decisionRef.current) {
      onDecide(decisionRef.current);
    }
  };
  ...
  return (
    <div style={{ animation: "fadeSlideUp 0.4s ease both" }}>
      ...
      {phase === "explanation" && !loadingExpl && explanation && (
        <ExplanationPanel sections={explanation} onContinue={handleContinue} />
      )}
      {phase === "explanation" && !loadingExpl && !explanation && (
        <button
  ...
  return (
    <div style={{ animation: "fadeSlideUp 0.5s ease both" }}>
      ...
      <div>
        ...
        {decisions.map((d, i) => {
          const chosenEV = d.chosenEV != null ? d.chosenEV : 0;
          const roomTitle = d.roomTitle || `Chamber ${i + 1}`;
          return (
            <div
              key={i}
              style={{
...
export default function DDMDungeon() {
  ...
  const handleDecide = useCallback(
    ...
    [decisions, roomIdx, rooms.length],
  );
